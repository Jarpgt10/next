...
export default function FooterMenuItem({ path, name, icon }) {
    const { shoppinCar, favorites } = useContext(DashboardContext);
    const currentPath = usePathname();
    const count = path === "/dashboard/shopping" ? shoppinCar.length
        : path === "/dashboard/favorites" ? favorites.length : 0;
    ...
    return (
        ...
        >
            {path !== '/dashboard/menu' && (<TagCounter number={count} />)}
